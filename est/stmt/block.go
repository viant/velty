...
func newStmt2(args []est.Compute) *stmt2 {
	return &stmt2{Compute: args[1],
		stmt1: stmt1{
			Compute: args[0],
		},
	}
...
func newStmt3(args []est.Compute) *stmt3 {
	return &stmt3{Compute: args[2],
		stmt2: stmt2{
			Compute: args[1],
			stmt1: stmt1{
				Compute: args[0],
...
func (s *stmt3) compute(state *est.State) unsafe.Pointer {
	s.stmt2.compute(state)
	return s.Compute(state)
}
...
func NewBlock(stmtsNew []est.New) est.New {
	...
	return func(control est.Control) (est.Compute, error) {
		...
		case 2:
			return newStmt2(stmts).compute, nil
		case 3:
			return newStmt3(stmts).compute, nil
		case 4:
